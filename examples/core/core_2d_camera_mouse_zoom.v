...
fn main()
{
	...
	mut camera := r.Camera2D{zoom: 1.0}
	...
	for !r.window_should_close() 
	{
		...
		match zoom_mode
		{
			0 {
				...
				if wheel != 0
				{
					...
					mouse_world_pos := r.get_screen_to_world_2d(r.get_mouse_position(), camera)
		...
		r.begin_drawing()
			...
			if zoom_mode == 0
			{
				r.draw_text('Mouse left button drag to move, mouse wheel to zoom'.str, 20, 50, 20, r.darkgray)
			}
			else
			{
				r.draw_text('Mouse left button drag to move, mouse press and move to zoom'.str, 20, 50, 20, r.darkgray)
			}
		r.end_drawing()
